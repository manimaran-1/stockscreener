...
import supertrend_aroon_scanner as scanner

# Page Config
st.set_page_config(page_title="Supertrend + Aroon Scanner", page_icon="📈", layout="wide")

# Header
st.title("📈 Supertrend + Aroon Momentum Scanner")
st.markdown("Scan Nifty/BSE markets for trend momentum using the Fingrad Supertrend & Aroon combination strategy.")
...
st_length = st.sidebar.number_input("Supertrend Length", min_value=1, max_value=200, value=10)
st_multiplier = st.sidebar.number_input("Supertrend Multiplier", min_value=1.0, max_value=20.0, value=3.0, step=0.5)
st.sidebar.markdown("---")
aroon_length = st.sidebar.number_input("Aroon Oscillator Length", min_value=1, max_value=200, value=14)
...
st.sidebar.markdown("---")
force_refresh = st.sidebar.checkbox("🔄 Force Refresh Data", help="Check this to clear the cache and download fresh live data from Yahoo Finance.")

# --- RUN SCAN ---
if st.button("Run Supertrend/Aroon Scan", type="primary"):
    symbols = []
    ...
    if not symbols:
        ...
        st.session_state['res_14_Supertrend_Aroon.py'] = results_df
        st.session_state['ctx_14_Supertrend_Aroon.py'] = {'index': selected_index, 'tf': active_timeframe}

if 'res_14_Supertrend_Aroon.py' in st.session_state and st.session_state['res_14_Supertrend_Aroon.py'] is not None:
    results_df = st.session_state['res_14_Supertrend_Aroon.py'].copy()
    context = st.session_state.get('ctx_14_Supertrend_Aroon.py', {'index': 'Custom', 'tf': '1d'})
    ...
    if results_df is not None and not results_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"supertrend_aroon_scan_{context['index'].replace(' ', '_')}_{context['tf']}.csv",
            mime="text/csv"
        )
    else:
        if not show_all:
...
try:
    st.image("/home/mohan/.gemini/antigravity/scratch/dmi_scanner/supertrend_aroon_strategy.jpg", use_container_width=True)
except Exception:
    pass

st.markdown("""
This scanner identifies **Momentum** setups based on the **Supertrend & Aroon Indicator** strategy. 

### 🎯 Signal Generation
*   **🟢 Bullish Focus (Buy/Long):** **Supertrend** turns **Green**, AND **Aroon Up** crosses above **Aroon Down**.
*   **🔴 Bearish Focus (Sell/Short):** **Supertrend** turns **Red**, AND **Aroon Down** crosses above **Aroon Up**.

### 📊 Understanding the Table Columns
*   **Signal Type:** The type of momentum setup identified.
*   **Supertrend:** The physical value of the Supertrend.
*   **Aroon Up/Down:** The raw values of the Aroon Indicator (scaled 0-100).
*   **Best Method (Supertrend SL):** The mathematically optimal Stop Loss for this strategy is pinned tight to the Supertrend baseline trigger point.
""")
