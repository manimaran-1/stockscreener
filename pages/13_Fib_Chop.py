...
import fib_chop_scanner as scanner

# Page Config
st.set_page_config(page_title="Fibonacci + Chop Zone Scanner", page_icon="📈", layout="wide")

# Header
st.title("📈 Fibonacci + Chop Zone Pullback Scanner")
st.markdown("Scan Nifty/BSE markets for trend continuation pullbacks based on the 50% Fibonacci retracement level and Chop Zone momentum shift.")
...
fib_lookback = st.sidebar.number_input("Fibonacci Swing Lookback", min_value=10, max_value=200, value=50)
...
# --- RUN SCAN ---
if st.button("Run Fib/Chop Scan", type="primary"):
    symbols = []
    ...
    if not symbols:
        ...
        st.session_state['res_13_Fib_Chop.py'] = results_df
        st.session_state['ctx_13_Fib_Chop.py'] = {'index': selected_index, 'tf': active_timeframe}

if 'res_13_Fib_Chop.py' in st.session_state and st.session_state['res_13_Fib_Chop.py'] is not None:
    results_df = st.session_state['res_13_Fib_Chop.py'].copy()
    context = st.session_state.get('ctx_13_Fib_Chop.py', {'index': 'Custom', 'tf': '1d'})
    ...
    if results_df is not None and not results_df.empty:
        st.success(f"Scan complete! Found {len(results_df)} Pullback setup(s).")
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"fib_chop_scan_{context['index'].replace(' ', '_')}_{context['tf']}.csv",
            mime="text/csv"
        )
    else:
        if not show_all:
...
try:
    st.image("/home/mohan/.gemini/antigravity/scratch/dmi_scanner/fib_chop_strategy.jpg", use_container_width=True)
except Exception:
    pass

st.markdown("""
This scanner identifies **Pullback and Trend Continuation** setups based on the **Fibonacci & Chop Zone** strategy. 

### 🎯 Signal Generation
*   **🟢 Bullish Focus (Buy/Long):** Price pulls back to the **50% Fibonacci level** while the long-term trend is bullish. The **Chop Zone** shifts from Blue/Green to Red (dropping below 38.2), signaling that the trend is regaining strength.
*   **🔴 Bearish Focus (Sell/Short):** Price bounces to the **50% Fibonacci level** while the long-term trend is bearish. The **Chop Zone** shifts to Red, signaling the continuation of the downtrend.

### 📊 Understanding the Table Columns
*   **Signal Type:** The type of pullback identified.
*   **Fib 50:** The exact price point of the 50% retracement level.
*   **Chop Zone:** The raw Chop Zone value. A value < 38.2 is 'Red' and indicates a strong, directional trend.
*   **Best Method (Fib SL):** The mathematically optimal Stop Loss for this strategy is placed at the **61.8% / 38.2% Fibonacci retracement level**.
""")
