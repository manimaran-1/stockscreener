...
# Header
st.title("📈 Bollinger Bands + MACD Momentum Scanner")
st.markdown("Scan Nifty/BSE markets for MACD confirmation at Bollinger Band extremes based on Fingrad VWMA crossover strategies.")
...
bb_length = st.sidebar.number_input("Bollinger Band Length", min_value=1, max_value=200, value=20)
bb_std = st.sidebar.number_input("Bollinger Std Dev", min_value=0.1, max_value=10.0, value=2.0, step=0.1)
st.sidebar.markdown("---")
...
# --- RUN SCAN ---
if st.button("Run BB/MACD Scan", type="primary"):
    symbols = []
    ...
    if not symbols:
        ...
        st.session_state['res_8_BB_MACD.py'] = results_df
        st.session_state['ctx_8_BB_MACD.py'] = {'index': selected_index, 'tf': active_timeframe}

if 'res_8_BB_MACD.py' in st.session_state and st.session_state['res_8_BB_MACD.py'] is not None:
    results_df = st.session_state['res_8_BB_MACD.py'].copy()
    context = st.session_state.get('ctx_8_BB_MACD.py', {'index': 'Custom', 'tf': '1d'})
    ...
    if results_df is not None and not results_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"bb_macd_scan_{context['index'].replace(' ', '_')}_{context['tf']}.csv",
            mime="text/csv"
        )
    else:
        if not show_all:
...
try:
    st.image("/home/mohan/.gemini/antigravity/scratch/dmi_scanner/bb_macd_strategy.png", use_container_width=True)
except Exception:
    pass
    
st.markdown("""
This scanner identifies **Reversal and Momentum** setups based on the **Bollinger Bands & MACD** strategy. It isolates potential turning points when price hits volatility extremes (Bollinger Bands) confirmed by momentum crossovers (MACD).

### 🎯 Signal Generation
*   **🟢 Bullish Signal (Buy/Long):** The price touches or closes near the **Lower Bollinger Band**, AND the **MACD Line** crosses *above* the **Signal Line**.
*   **🔴 Bearish Signal (Sell/Short):** The price touches or closes near the **Upper Bollinger Band**, AND the **MACD Line** crosses *below* the **Signal Line**.

### 📊 Understanding the Table Columns
*   **Signal:** The entry triggered by the Bollinger Band touch and MACD crossover (Bullish=Buy, Bearish=Short).
...
*   **Lower/Upper BB:** The volatility bands derived from standard deviation.
*   **Best TP / SL:** Uses the opposite Bollinger Band as a logical Take Profit target, and the recent pivot extreme as a standard Stop Loss.
