...
import datetime
import data_loader
import bb_cci_scanner as scanner
st.set_page_config(page_title="Bollinger Bands & CCI Scanner", page_icon="⚙️", layout="wide")
...
st.title("📉 Bollinger Bands & CCI Strategy Scanner")
st.markdown("Scan NSE/BSE markets for setups using Price Action around Bollinger Bands combined with CCI Oversold/Overbought levels.")
...
bb_len = st.sidebar.number_input("Bollinger Bands Length", min_value=1, value=20)
bb_std = st.sidebar.number_input("Bollinger Bands Std Dev", min_value=0.1, value=2.0)
cci_len = st.sidebar.number_input("CCI Length", min_value=1, value=14)
...
if st.button("Run BB + CCI Scan", type="primary"):
    symbols = []
    ...
    if not symbols:
        ...
        settings = {'bb_length': bb_len, 'bb_std': bb_std, 'cci_length': cci_len}
        ...
        progress_bar.empty()
        status_text.empty()
        st.session_state['res_19_BBCCI'] = results_df
if 'res_19_BBCCI' in st.session_state and st.session_state['res_19_BBCCI'] is not None:
    results_df = st.session_state['res_19_BBCCI'].copy()
...
st.markdown("""
**Trading with Bollinger Bands & CCI**
**🟢 LONG POSITION**
*   **ENTRY:** 
    * Price touches or closes near the lower Bollinger Band.
    * CCI crosses above -100 from below the oversold Zone.
*   **TARGET / EXIT:**
    * Price touches or closes near the upper Bollinger Band.
    * CCI crosses below +100 from above overbought Zone.
**🔴 SHORT POSITION**
*   **ENTRY:** 
    * Price touches or closes near the upper Bollinger Band.
    * CCI crosses below +100 from above the overbought Zone.
*   **TARGET / EXIT:**
    * Price touches or closes near the lower Bollinger Band.
    * CCI crosses above -100 from below oversold Zone.
""")
