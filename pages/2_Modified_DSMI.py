...
st.set_page_config(page_title="Modified DSMI Dashboard", layout="wide")

st.title("Modified DSMI Scanner (Directional Strength & Momentum) ⚡")
st.markdown("Scan NSE stocks using an EMA-smoothed Directional Movement logic based on Uncle_the_shooter's Custom DSMI PineScript. Evaluates exact Trend Status.")
...
default_start = datetime.now() - pd.Timedelta(days=30)
...
st.sidebar.markdown("---")
st.sidebar.subheader("DSMI Math Parameters")
dsmi_length = st.sidebar.number_input("DSMI Period", min_value=1, max_value=100, value=20)
dsmi_entry = st.sidebar.number_input("DSMI Entry Level", min_value=1, max_value=100, value=20)

st.sidebar.markdown("---")
st.sidebar.subheader("Trend Strength Thresholds")
weak_thr = st.sidebar.number_input("Weak (Below = Neutral)", value=10)
neutral_thr = st.sidebar.number_input("Moderate (Up to)", value=35)
strong_thr = st.sidebar.number_input("Strong (Up to)", value=45)
overheat_thr = st.sidebar.number_input("Overheated (Up to)", value=55)
...
if st.button("Run DSMI Scan"):
    ...
    if symbols:
        ...
        status_text.text(f"Starting scan for {total_symbols} symbols on {active_timeframe} timeframe...")
        ...
        # Only pre-filter if we are scanning a large universe and trying to find hits
        if len(symbols) > 10 and not show_all and active_timeframe == '1d':
            with st.spinner(f"Pre-filtering {len(symbols)} symbols via TradingView API (Speed Optimization)..."):
                filtered_symbols = data_loader.fetch_dmi_prefilter(symbols)
                
                if not filtered_symbols:
                    st.warning("No symbols passed the initial DMI pre-filter (Lines are too far apart).")
                    filtered_symbols = [] # Prevent crash
                else:
                    st.info(f"⚡ Pre-filter reduced universe from {len(symbols)} to {len(filtered_symbols)} candidates.")
                    symbols = filtered_symbols

        # We delegate to scanner.scan_market_dsmi which handles the massive bulk data fetch optimization automatically
        with st.spinner("Processing..."):
            results_df = scanner.scan_market_dsmi(symbols, active_timeframe, start_date, end_date, show_all, dsmi_length, weak_thr, neutral_thr, strong_thr, overheat_thr, dsmi_entry, force_refresh_token=force_refresh_token, progress_callback=update_scan_progress)
        
        progress_bar.empty()
        status_text.empty()

        st.session_state['res_2_Modified_DSMI.py'] = results_df
        st.session_state['ctx_2_Modified_DSMI.py'] = {'index': selected_index, 'tf': active_timeframe}

if 'res_2_Modified_DSMI.py' in st.session_state and st.session_state['res_2_Modified_DSMI.py'] is not None:
    results_df = st.session_state['res_2_Modified_DSMI.py'].copy()
    context = st.session_state.get('ctx_2_Modified_DSMI.py', {'index': 'Custom', 'tf': '1d'})
    ...
    if results_df is not None and not results_df.empty:
        ...
        st.download_button(
            ...
            csv,
            "modified_dsmi_scan_results.csv",
            "text/csv",
            key='download-csv'
        )
    else:
