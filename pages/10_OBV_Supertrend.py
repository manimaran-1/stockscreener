...
import obv_supertrend_scanner as scanner

# Page Config
st.set_page_config(page_title="OBV + Supertrend Scanner", page_icon="📊", layout="wide")

# Header
st.title("📊 OBV + Supertrend Momentum Scanner")
st.markdown("Scan Nifty/BSE markets for OBV and Supertrend Breakout momentum trades based on Fingrad OBV & Supertrend strategy.")
...
# --- RUN SCAN ---
if st.button("Run OBV/Supertrend Scan", type="primary"):
    symbols = []
    ...
    if not symbols:
        ...
        st.session_state['res_10_OBV_Supertrend.py'] = results_df
        st.session_state['ctx_10_OBV_Supertrend.py'] = {'index': selected_index, 'tf': active_timeframe}

if 'res_10_OBV_Supertrend.py' in st.session_state and st.session_state['res_10_OBV_Supertrend.py'] is not None:
    results_df = st.session_state['res_10_OBV_Supertrend.py'].copy()
    context = st.session_state.get('ctx_10_OBV_Supertrend.py', {'index': 'Custom', 'tf': '1d'})
    ...
    if results_df is not None and not results_df.empty:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"obv_supertrend_scan_{context['index'].replace(' ', '_')}_{context['tf']}.csv",
            mime="text/csv"
        )
    else:
        if not show_all:
...
try:
    st.image("/home/mohan/.gemini/antigravity/scratch/dmi_scanner/obv_supertrend_strategy.jpg", use_container_width=True)
except Exception:
    pass

st.markdown("""
This scanner identifies **Momentum and Trend-Following** setups based on the **On Balance Volume (OBV) & Supertrend** strategy. 

### 🎯 Signal Generation
*   **🟢 Bullish Signal (Buy/Long):** **Supertrend** turns **Green**, AND **OBV** is rising (which confirms Buying pressure).
*   **🔴 Bearish Signal (Sell/Short):** **Supertrend** turns **Red**, AND **OBV** is falling (which confirms Selling pressure).
...
*   **Supertrend:** The Supertrend baseline value.
...
*   **Best Method (Signal ± ATR):** Stop Loss and Take Profit calculated using `Signal Price ± ATR`.
""")
