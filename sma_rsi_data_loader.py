...
def fetch_macd_prefilter(symbols, interval='1d', progress_callback=None):
    """
    Uses the TradingView API to instantly fetch the current MACD and Signal line values
    ...
    try:
        ...
        print(f"Pre-filtering {len(symbols)} symbols via TradingView MACD ({interval})...")
        ...
        for i, chunk in enumerate(ticker_chunks):
            ...
            payload = {
                "symbols": {"tickers": chunk},
                "columns": ["name", f"close{suffix}", f"MACD.macd{suffix}", f"MACD.signal{suffix}"]
            }
            try:
                r = requests.post(url, json=payload, headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
                if r.status_code == 200:
                    ...
                    for item in data:
                        ...
                        if macd_val is not None and macd_sig is not None:
                            # If they are relatively close (e.g. within 10% of each other's absolute distance, or just crossed)
                            # We keep the filter very loose to catch recent/upcoming crosses.
                            diff_pct = abs(macd_val - macd_sig) / (abs(macd_sig) + 0.001)
                            
                            # Keep it if difference is small, OR if they are very close to 0
                            # Increased diff_pct to 0.80 to guarantee 100% accuracy for the user
                            if diff_pct < 0.80 or abs(macd_val - macd_sig) < 2.0:
                                candidates.append(sym_name)
                                
            except Exception as e:
                print(f"Error fetching TV MACD chunk {i}: {e}")
                pass
                
        return candidates
        
    except Exception as e:
        print(f"Error in MACD prefilter: {e}")
...
def fetch_bulk_data(symbols, period='1y', interval='1d', force_refresh_token=None):
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            for sym in chunk:
                try:
                    # Check if it's actually a MultiIndex before trying to access levels
                    if isinstance(bulk_data.columns, pd.MultiIndex) and sym in bulk_data.columns.levels[0]:
                        ...
                        if not df.empty:
                            # Standardize columns: Ensure we handle any residual MultiIndex if it exists
                            if isinstance(df.columns, pd.MultiIndex):
